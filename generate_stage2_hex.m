...
parfor i=1:length(hexparameters)
para=hexparameters(:,i)';
vel_plane=hex(para(1)).velocity(para(2),:);
% select 60 degreee plane in titanium (Hearmon)
%ke_vel= vel_plane;
vel = vel_plane;%velocity model for one plane (1,181)
velrot =circshift(vel,para(3));
%rawdata_interval = rawdatainterval(j); %step between 
rawdata_interval =para(4);
lambda = 24e-6; %Acoutsic wavelength
no_fringes = para(5); %number of fringes used in the acoutic generation patch
...
random_noise_proportion = para(7);
...
name=hex_name(:,para(1));
filename=strcat(root,name,"-vel",string(para(2)),"-ro",string(para(3)), ...
